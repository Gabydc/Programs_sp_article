...
dir='/home/wagm/cortes/Localdisk/Results/16_08/12/';
%dir='/dev/media/Sphinx/Doctorado_Delft/Results/16_06/06/';
folder=[num2str(def) 'size_'   num2str(nx) 'perm_' num2str(per) '_5wells_' num2str(defvect) '_defvect' 'c_' num2str(fc) 'e-3' extra];
mkdir([dir], folder)
dir = [dir folder '/'];
...
pressi = ode23(@(z,p) g .* rhoi(p), [z_0, z_max], p_r);
...
pressureEqi = @(p, p0, dt) (1/dt) .* (pv(p).*rhoi(p) - pv(p0).*rhoi(p0)) ...
    - div( avg(rhoi(p) ./ mu) .* T .* grad(p - g*rhoi(p).*z) );
...
for i=1:5
    wellRatesi{i}=@(p, bhp) W(i).WI .* (rhoi(p(W(i).cells)) ./ mu) .* (bhp - p( W(i).cells) + g*W(i).dZ.*rhoi(p( W(i).cells)));
end
...
while t < totTime,
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        eqs{1}(W(1).cells) = eqs{1}(W(1).cells) - wellRatesi{1}(output{1}, output{2});
        eqs{1}(W(2).cells) = eqs{1}(W(2).cells) - wellRatesi{2}(output{1}, output{3});
        eqs{1}(W(3).cells) = eqs{1}(W(3).cells) - wellRatesi{3}(output{1}, output{4});
        eqs{1}(W(4).cells) = eqs{1}(W(4).cells) - wellRatesi{4}(output{1}, output{5});
        eqs{1}(W(5).cells) = eqs{1}(W(5).cells) - wellRatesi{5}(output{1}, output{6});
        ...
        if vwells==1
                    eqs{7} = output{7} - sum(wellRatesi{1}(output{1}, output{2}))/rhoS;
        eqs{8} = output{8} - sum(wellRatesi{2}(output{1}, output{3}))/rhoS;
        eqs{9} = output{9} - sum(wellRatesi{3}(output{1}, output{4}))/rhoS;
        eqs{10} = output{10} - sum(wellRatesi{4}(output{1}, output{5}))/rhoS;
        eqs{11} = output{11} - sum(wellRatesi{5}(output{1}, output{6}))/rhoS;
        else
...
%         end
  ...
  %max(res+J*xi)
%              pause
             %[upd,h,h2,h21]=ICCG_0(-J,res,xi,200,e^-7,l,0);
...
%  fclose(fileID);
        ...
        zout(Ix{i},dv)=output{i}.val;
                ...
                if pod==1
                if step==defvect+1
                    [U,S,hpod1]=defpodf_D(zupd,dir);
                end
                z=U(:,5:10);
                else z=zupd;
                end
                [upd,ee]=DICCG_te(-J,res,xi,200,10^-5,z,l,eig,step,dir,nit);
                     for i=1:11
                     output{i}.val   = output{i}.val   + upd(Ix{i});
                    end
            ne=length(ee);
