...
%dir='/dev/media/Sphinx/Doctorado_Delft/Results/16_08/20/';
...
% hp=plotCellData(G, rock.perm);
 ...
 fine = [nxf nyf nzf];
 coarse = [nx ny nz];
  name=['SPE10_' num2str(nx) '_' num2str(ny)  '_5wells_' 'c_' num2str(fc) 'e-3' extra];
  mkdir([dir], name )
  dir=[dir name '/'];
...
clf
hp=plotCellData(G, log(rock.perm));
%
%  view(0,90)
...
%         pf(nns+1)=sol(step+1).pressure(ns+1);
%         nns=nns+1;
%         ns=(nns)*nx+nns+1;
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        if norm(res) < tol
            ...
            if  def==0
                ...
                [upd,ee]=ICCG_t(-J,res,xi,200,tolls,l,dir,eig,step,nit);
                ne=length(ee);
                miter(step,nit+1)=ne;
                %ee=0;
                fileID = fopen(text,'a');
                ...
                if step <dvec+1
                    ...
                    [upd,ee]=DICCG_te(-J,res,xi,200,tolls,z,l,eig,step,dir,nit,dvec);
                    for i=1:11
                        output{i}.val   = output{i}.val   + upd(Ix{i});
                    end
                    ne=length(ee);
