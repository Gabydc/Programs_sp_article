...
dir='/dev/media/Sphinx/Doctorado_Delft/Results/16_04/04_21/';
folder=['size_'   num2str(nx) 'perm_' num2str(per) '_5wells' ];
mkdir([dir], folder)
dir = [dir folder '/'];
% for i=2:2:6
%  %for i=2
...
while t < totTime,
    ...
    while (resNorm > tol) && (nit < maxits)
       ...
       if step <6
        l=ichol(J);
        e=0;
       % [upd,hc0,hc01,hc02,iterIC,leginf]=ICCG_t(-J,res,xi,200,10^-7,l,e,step,nit+1,leginf);
...
fprintf(fileID,' %6d  &   %6d  &  %6d \\\\ \n', step, nit+1,ICiter );
 fclose(fileID);
        zd(:,step)=upd;
           ...
           [upd,iterD,ee,hl1,h2,h21]=DICCG_t(-J,res,xi,200,10^-7,S,l,e,5,79);
           fileID = fopen(text,'a');
 %fprintf(fileID,'Time step', 'Newton iteration', '# Iterations solver \\');
 fprintf(fileID,' %6d  &   %6d   & %6d \\\\  \n ', step, nit+1, iterD);
 fclose(fileID);
...
pause
    ...
    if nit > maxits,
        ...
        plot(xaxis, y1,'x', xaxis,y2,'r*', xaxis,y3,'bo', xaxis,y4,'gp', xaxis,y5,'ms');
