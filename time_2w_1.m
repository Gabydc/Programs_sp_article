...
while t < totTime,
    ...
    %    upd = -(J \ res); % Newton update
       ...
       resNorm = norm(res);
        ...
        b{step}=res;
        xs{step}=xi;      
        residual=b{step}-a{step}*xi;
              nr=sqrt(residual'*residual)
              if step==1
                  nr0=sqrt(residual'*residual);
              end
              nr0=nr0
             nb=sqrt(b{step}'*b{step})        
            rb= nr/nb
             rr0=nr/nr0
             pause
             %if resNorm>tol
             [upd,hc0,hc01,hc02]=ICCG_0(-J,res,xi,iter,tolc,l,e);
             residual=b{step}-a{step}*upd;
              rub=sqrt(residual'*residual)/sqrt(b{step}'*b{step}) 
              ru0=sqrt(residual'*residual)/nr0
              pause
      %  Update variables
        ...
        %resupd = upd
       % residualupd=b{step}-a{step}*xf{step};
%         nru=sqrt(residualupd'*residualupd)
%                     rbu= nru/nb 
%              rr0u=nru/nr0
              updc=upd;
%        pause
