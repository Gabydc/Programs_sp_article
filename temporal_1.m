...
wc{1} = W(1).cells; % perforation grid cells
WI{1} = W(1).WI;    % well indices
dz{1} = W(1).dZ;    % perforation depth relative to well reference depth

wc{2} = W(2).cells; % perforation grid cells
WI{2} = W(2).WI;    % well indices
dz{2} = W(2).dZ;    % perforation depth relative to well reference depth
...
% rate.
[p_ad, bhp_ad{1}, bhp_ad2{2}, qS_ad{3}, qS_ad2{4}] = initVariablesADI(p_init, p_init(wc{1}), p_init(wc{2}), 0, 0);
%break
