...
per=2;
for i= 1:2:5
    rock.perm(1+(i-1)*lsz*nx:i*lsz*nx)  = 3*10^(-per)*milli*darcy();
end
%dir='/home/wagm/cortes/Localdisk/Results/16_05/05_09/';
dir='/dev/media/Sphinx/Doctorado_Delft/Results/16_05/05_04/';
folder=['size_'   num2str(nx) 'perm_' num2str(per) '_5wells_' num2str(defvect) '_defvect' num2str(def) '_2'];
mkdir([dir], folder)
dir = [dir folder '/'];
% for i=2:2:6
%  %for i=2
...
while t < totTime,
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        if step <defvect+1 || (step >9 && step<16)
            ...
            l=ichol(J);
            e=0;
            [upd,ee]=ICCG_t(-J,res,xi,200,10^-7,l);
            ne=length(ee);
            miter(step,nit+1)=ne;
            iter=1:ne;
%            figure(800)
...
for i=1:4
    ...
    if def==1
        ...
        hold on
        hn=plot(10:15,miter(10:15,i),'r*');
        hold on
        subplot(2,2,i)
        hn=plot(defvect+1:9,miter(defvect+1:9,i),'bp');
        hold on
        hn=plot(16:numSteps,miter(16:numSteps,i),'bp');
        hold on
        legend('ICCG','DICCG');
    else
        subplot(2,2,i)
