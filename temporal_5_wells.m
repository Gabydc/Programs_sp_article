...
colorbar; view(30,50); axis equal
camproj perspective
...
while t < totTime,
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        eqs{2} = bhp_ad - 0*barsa;
        eqs{3} = bhp_ad2 - 0*barsa;
        eqs{4} = bhp_ad3 - 0*barsa;
        eqs{5} = bhp_ad4 - 0*barsa;
        % Final equation is prescribed bhp at injector
        eqs{6} = bhp_ad5 -600*barsa;
...
%         upd = -(J \ res); % Newton update
        ...
        e=0;
        iter=200;
        [upd,hc0,hc01,hc02]=ICCG_0(-J,res,xi,iter,10^-7,l,e); 
        upd=-upd;
      %  Update variables
