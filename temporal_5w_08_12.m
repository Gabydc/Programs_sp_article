...
dir='/home/wagm/cortes/Localdisk/Results/16_09/05/';
    ...
    while (resNorm > tol) && (nit < maxits)
        ...
        if norm(res) < tol
            ...
            if  def==0
                ...
                [upd,ee,md]=ICCG_te1(-J,res,xi,200,10^-5,l,dir,eig,step,nit,md);
                ne=length(ee);
                miter(step,nit+1)=ne;
                %ee=0;
                fileID = fopen(text,'a');
                ...
                if step <dvec+1
                    ...
                    [upd,ee]=DICCG_te1(-J,res,xi,200,10^-5,z,l,eig,step,dir,nit,dvec,md);
                    for i=1:11
                        output{i}.val   = output{i}.val   + upd(Ix{i});
                    end
                    ne=length(ee);
