function [xf,ee,md,hls]=ICCG_te1(a,b,xi,iteration,tol,l,dir,eig,step,nit,md)
n=length(a);
hls=0;
plotStyle = {'>','v','<','^','*','o','+'};
...
md(2)=max(Da);
axis('tight')
title('Eigenvalues M^{-1}J','FontSize',16)
...
figure(81)
clf
h21=semilogy(abs(D),'b*');
m1=min(D);
m2=max(D);
%axis([1 n m1 m2])
axis('tight')
title('Eigenvalues J','FontSize',16)
ylabel('log(value)','FontSize',16)
xlabel('Eigenvalue','FontSize',16)
hold on   
...
for iter=1:iteration 
    ...
    if nit<4
        subplot(2,2,nit+1)
        hls=plot(step,ee(iter),plotStyle{mod(step,6)+1},'Color', colors(mod(step,6)+1,:));
        hold on
        title( ['LS error, NR Iteration ' num2str(nit)] )
    end
